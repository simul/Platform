...
namespace simul
{
	namespace gles
	{
		...
		//! GLES renderplatform implementation
		class SIMUL_GLES_EXPORT RenderPlatform:public crossplatform::RenderPlatform
		{
		public:
			            RenderPlatform();
			...
			{
				return crossplatform::RenderPlatformType::GLES;
			}
			virtual const char *GetSfxConfigFilename() const override
			{
				return "Sfx/GLES.json";
			}
			...
			void									SetUnorderedAccessView(crossplatform::DeviceContext& deviceContext, const crossplatform::ShaderResource& res, crossplatform::Texture* tex, int index, int mip) override;
			...
			//! Returns the format that GLES stores the texture data internally. e.g. GL_RGBA8
			static                                  GLuint ToGLInternalFormat(crossplatform::PixelFormat p);
			//! Returns the crossplatform::PixelFormat from the GLES internal format. e.g PixelFormat::RGBA_8_UNORM
			static                                  crossplatform::PixelFormat FromGLInternalFormat(GLuint p);
			//! Return the format that GLES uses the load in pixel data. e.g GL_RGBA
            ...
            gles::Texture*                        GetDummy2D();
            //! Returns 3D dummy texture 1 white texel
            gles::Texture*                        GetDummy3D();
        ...
        protected:
            ...
            gles::Texture*    mDummy2D;
            gles::Texture*    mDummy3D;
