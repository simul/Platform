...
bool FramebufferGL::CreateBuffers()
{
	...
	for(int j=0;j<faces;j++)
	{
		glBindFramebuffer(GL_FRAMEBUFFER,*f);
		{
			...
			auto glcolour = (gles::Texture*)buffer_texture;
			auto gldepth = (gles::Texture*)buffer_depth_texture;
...
void FramebufferGL::Activate(crossplatform::GraphicsDeviceContext& deviceContext)
{
	...
	colour_active   = true;
	GLenum target = numAntialiasingSamples == 1 ? GL_TEXTURE_2D : GL_TEXTURE_2D_MULTISAMPLE;
    gles::Texture* glcol      = (gles::Texture*)buffer_texture;
    gles::Texture* gldepth    = nullptr;
    if (buffer_depth_texture)
    {
        gldepth         = (gles::Texture*)buffer_depth_texture;
