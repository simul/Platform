//This is the Platform local copy of Platform\External\imgui\backends\imgui_impl_win32.cpp
// It should be kept up-to-date with that file, but has modifications to allow e.g. 3D interfaces with non-standard mouse positions.
...
void ImGui_ImplWin32_SetFunction_GetCursorPos(int(*f)(long[]))
{
	getCursorPos=f;
}
...
static void ImGui_ImplWin32_UpdateMouseData()
{
    ...
    bool has_mouse_screen_pos =false;
	if(getCursorPos)
		has_mouse_screen_pos=getCursorPos((long*)&mouse_screen_pos) != 0;
	else
    ...
    if (is_app_focused)
    {
        ...
        if (!io.WantSetMousePos && !bd->MouseTracked && has_mouse_screen_pos)
        {
            ...
            if (!(io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)&&!getCursorPos)
...
ImGuiKey ImGui_ImplWin32_VirtualKeyToImGuiKey(unsigned long long wParam)
{
    switch (wParam)
    {
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
    {
		...
		// Ignore if we're overriding the cursor pos.
		if(!getCursorPos)
		{
