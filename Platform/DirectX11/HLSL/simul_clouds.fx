...
RaytracePixelOutput PS_Raytrace(RaytraceVertexOutput IN)
{
	...
	//int i=40;
	{
		...
		{
			float2 noise_texc	=noise_texc_0*layer.layerDistance/fractalRepeatLength+layer.noiseOffset;
...
RaytracePixelOutput PS_RaytraceForward(RaytraceVertexOutput IN)
{
	...
	for(int i=0;i<layerCount;i++)
	{
		...
		float dist				=layer.layerDistance;
		float z					=saturate(dist/maxFadeDistanceMetres);
		float3 world_pos		=viewPos+dist*view;
		world_pos.z				-=layer.verticalShift;
		float3 texCoords		=(world_pos-cornerPos)*inverseScales;
