...
{
	float3 output=colour.rgb*flareTexture.Sample(flareSamplerState,float2(.5f,.5f)+0.5*IN.tex).rgb;
...
{
	vec4 result=flareTexture.Sample(flareSamplerState,float2(.5f,.5f)-0.5*IN.tex);
	// IN.tex is +- 1.
	float3 normal;
	normal.x=IN.tex.x;
	normal.y=IN.tex.y;
...
technique11 stars
{
    pass p0 
    {
		...
		SetBlendState(AddBlend, vec4(1.0,1.0,1.0,1.0 ), 0xFFFFFFFF );
    }
}
