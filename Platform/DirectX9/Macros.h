...
#define BreakIfDebugging()
	...
	#ifndef VOID_RETURN
		#define VOID_RETURN(x)	{ hr = x; if( FAILED(hr) ) {wchar_t text[200];wsprintf(text,L"V_RETURN error %s at file %s, line %d",GetErrorText(hr),__WFILE__,__LINE__);std::cout<<text<<std::endl;MessageBox(NULL,text,L"ERROR", MB_OK|MB_SETFOREGROUND|MB_TOPMOST);BreakIfDebugging();return; } }
	#endif
	#ifndef V_RETURN
		#define V_RETURN(x)	{ hr = x; if( FAILED(hr) ) {wchar_t text[200];wsprintf(text,L"V_RETURN error %s at file %s, line %d",GetErrorText(hr),__WFILE__,__LINE__);std::cout<<text<<std::endl;MessageBox(NULL,text,L"ERROR", MB_OK|MB_SETFOREGROUND|MB_TOPMOST);BreakIfDebugging();return hr; } }
	#endif
	#ifndef V_CHECK
		#define V_CHECK(x)	{ hr = x; if( FAILED(hr) ) {wchar_t text[200];wsprintf(text,L"V_CHECK error %s at file %s, line %d",GetErrorText(hr),__WFILE__,__LINE__);std::cout<<text<<std::endl;MessageBox(NULL,text,L"ERROR", MB_OK|MB_SETFOREGROUND|MB_TOPMOST);BreakIfDebugging();} }
	#endif
	#ifndef V_FAIL
...
#else
	#ifndef V_RETURN
		#define V_RETURN(x)	{ hr = x; if( FAILED(hr) ) {std::cerr<<"V_RETURN error "<<GetErrorText(hr)<<" at file "<<__FILE__<<" line "<<__LINE__<<std::endl;return hr; } }
	#endif
	#ifndef V_CHECK
		#define V_CHECK(x)	{ hr = x; if( FAILED(hr) ) {std::cerr<<"V_RETURN error "<<GetErrorText(hr)<<" at file "<<__FILE__<<" line "<<__LINE__<<std::endl; } }
	#endif
	#ifndef V_FAIL
