								...
								,float threshold)
{
	...
	vec3 to_source			=(source_pos_w-viewPosKm);
	float dist_to_source	=length(to_source);
	vec3 dir_to_source		=normalize(to_source);

	float cosine			=dot(view,dir_to_source);
	if(cosine<maxCosine)
		discard;

	// The longest distance from the camera that will be illuminated is the solution to
	// (X^2+R^2-2XRcosine=Y^2
	// Where dist_to_source=X,maxIlluminatedRadius=Y, and R is the distance we are seeking. We want the larger of the two solutions.
	// so R^2 + (-2 X cos) R +(X^2-Y^2)=0;
	float BB=-2.0*dist_to_source*cosine;
	float CC=dist_to_source*dist_to_source-maxIlluminatedRadius*maxIlluminatedRadius;
	float U=BB*BB-4.0*CC;
	if(U<0.0)
		discard;
	float maxDistanceKm	=min(maxCloudDistanceKm,0.5*(-BB+sqrt(U)));
	...
	vec3 gridOriginPosKm				=cornerPosKm+0.25/inverseScalesKm.z;
	...
	float distanceKm			=distance(world_pos,viewPosKm);
	int3 c							=c0;
	...
	{
		world_pos					+=view;
		if((view.z<0&&world_pos.z<min_z)||(view.z>0&&world_pos.z>max_z)||distanceKm>maxDistanceKm||solidDist_nearFar.y<lastFadeDistance)
			break;
		...
		world_pos					+=d*view;
		vec3 cloudWorldOffset		=world_pos-cornerPosKm;
		vec3 cloudTexCoords			=(cloudWorldOffset)*inverseScalesKm;
		c							+=c_step;
