											...
											,bool consistent_steps)
{
	...
	for(int i=0;i<768;i++)
	{
		...
		if(fade>0)
		{
			vec4 density = sample_3d_lod(cloudDensity, wwcSamplerState, cloudTexCoords, fadeDistance*4.0);
			//if(!found)
			{
			...
			//if(found)
			{
				...
				calcDensity(cloudDensity,cloudLight,cloudTexCoords,noiseval,fractalScale,fadeDistance,density,light);
				density.z*=fade;
				if(do_rain_effect)
				{
