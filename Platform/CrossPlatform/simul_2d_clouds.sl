...
vec4 Clouds2Dunfaded(vec3 view,float cos0,vec2 texc_global,vec2 texc_detail,float dist,float cloudInterp,vec3 sunlight,vec4 lightResponse)
{
...
vec4 Clouds2DPS(vec2 texc_global,vec2 texc_detail,vec3 wEyeToPos,float dist,float cloudInterp,vec3 sunlight,vec3 lightDir,vec4 lightResponse)
{
	...
	float sine			=view.z;
	float cos0			=dot(normalize(lightDir),view);
	vec4 final			=Clouds2Dunfaded(view,cos0,texc_global,texc_detail,dist,cloudInterp,sunlight,lightResponse);
