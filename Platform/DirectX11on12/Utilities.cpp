...
#include "Simul/Platform/DirectX11on12/DX11Exception.h"
...
bool simul::dx11on12::IsTypeless( DXGI_FORMAT fmt, bool partialTypeless )
{
...
DXGI_FORMAT simul::dx11on12::TypelessToSrvFormat(DXGI_FORMAT fmt)
{
...
void ArrayTexture::create(ID3D11Device *pd3dDevice,const std::vector<std::string> &texture_files,const std::vector<std::string> &pathsUtf8)
{
	...
	{
		textures.push_back(simul::dx11on12::LoadStagingTexture(pd3dDevice,texture_files[i].c_str(),pathsUtf8));
	}
