...
class ShaderUploadTable : public GpuUploadBuffer
{
    ...
    ShaderUploadTable() {}
public:
    ShaderUploadTable(ID3D12Device* device, UINT numShaderRecords, UINT shaderRecordSize, LPCWSTR resourceName = nullptr) 
        : m_name(resourceName)
    {
        m_shaderRecordSize = Align(shaderRecordSize, D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
        m_shaderRecords.reserve(numShaderRecords);
        m_BufferSize = numShaderRecords * m_shaderRecordSize;
        Allocate(device, m_BufferSize, resourceName);
