...
namespace platform
{
	namespace math
	{
		...
		/// Velocity is a platform::math::Vector3 which can be accessed using #GetVelocity().
		/// Angular velocity is a platform::math::Vector3 which can be accessed using #GetAngularVelocity().
		class SIMUL_MATH_EXPORT SimulReferenceFrame: public SimulOrientation
		{
		public:
			SimulReferenceFrame();
			void Clear();
			platform::math::Vector3 Ve;
			platform::math::Vector3 Omega;
		/// \name General
		///
			...
			platform::math::Vector3 GetLocalAngularVelocity() const;									///< Return the angular velocity in local co-ordinates.   
