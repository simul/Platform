...
#ifndef PLAYSTATION2
#ifndef SIMD
	M.Values[0 * 4 + 1] = -v.Values[2];
	M.Values[0 * 4 + 2] = v.Values[1];
	M.Values[1 * 4 + 0] = v.Values[2];
	M.Values[1 * 4 + 2] = -v.Values[0];
	M.Values[2 * 4 + 0] = -v.Values[1];
	M.Values[2 * 4 + 1] = v.Values[0];
#else
	__asm
		{
		mov edi,M
...
/*	for(i=0;i<TempV.size;i++)
	...
	asm __volatile__("
						 .set noreorder
							 add t4,
					 zero, % 4 blez t4, Escape // for zero height matrix
											nop add t9,
					 zero, % 0 // Start of V2.Values
							   xor t5,
					 t5				  // V2's value count
						 iLoop :	  // Rows of M1 and M
					 add t0, zero, t9 // Start of Row i of M
									   add t7,
					 zero, % 2							 // Row zero of M2
							   jLoop : add t1, zero, % 1 // Start of Row i of M1
														 add t3,
									   zero, % 3 // Width of M1 and M2
												 add t2,
									   zero, t7 // Row j of M2
												 vsub vf4,
									   vf0, vf0 blt t3, 4, Tail nop kLoop : lqc2 vf1, 0(t1) // Load 4 numbers into vf1
																			lqc2 vf2,
																			0(t2) // Load 4 numbers into vf2
																			vmul vf3,
																			vf1, vf2 vadd vf4, vf4, vf3 addi t1, 16 // next four numbers
																			addi t2,
																			16 addi t3, -4 // counter-=4
																						bgt t3,
																			3, kLoop nop
																				   // Now vf4 contains 4 sums to be added together.
																				   vaddx vf5,
																			vf4, vf4x vaddy vf5, vf4, vf5y vaddz vf4, vf4, vf5z // So the sum is in the w.
																															   beqz t3,
																			NoTail
																				Tail : lqc2 vf1, 0(t1)lqc2 vf2, 0(t2) // All of next 4 values
																					   vmul vf2,
																					   vf1, vf2 vaddx vf4, vf4, vf2x // w of vf1+ x of vf2
																													beq t3,
																					   1, NoTail nop vaddy vf4, vf4, vf2y // w of vf1+ y of vf2
																														 beq t3,
																					   2, NoTail nop vaddz vf4, vf4, vf2z							 // w of vf1+ z of vf2
																														 NoTail : vmul vf4, vf4, vf0 // Now it's 3 zeroes and the sum in the w slot.
																																					 // Now rotate into place.
																																					 and t6,
																																  t5, 0x3 //	where is the column in 4-aligned data?
																																		  and t8,
																																  t0, ~0xF //	4-align the pointer
																																	  beq t6,
																																  3, esccase // W
																																		 nop vmr32 vf4,
																																  vf4
																																  ...
																																  vf4 // X
																																	  ...
																																	  // nop
																																	  esccase : lqc2 vf6, 0(t8)vadd vf4, vf6, vf4							  // add to what's there already.
																																												  emptycase : sqc2 vf4, 0(t8) // store in t0
																																															  // blez t3,NoTail
																																															  // Tail:
																																															  ...
																																															  -1 // row counter
																																															  addi t5,
																																															  1 // where in 4-byte aligned V2 vector
																																															  bgtz t4,
																																															  iLoop
																																																  nop
																																																	  Escape : "					: 
	...
	asm __volatile__("
					 ...
					 zero, % 2							 // Row zero of M2
							   jLoop : add t1, zero, % 1 // Start of Row i of M1
									   ...
									   zero, t7 // Row j of M2
												 blt t3,
									   4, Tail vsub vf4, vf0, vf0 kLoop : lqc2 vf1, 0(t1) // Load 4 numbers into vf1
																		  lqc2 vf2,
																		  0(t2) // Load 4 numbers into vf2
																		  vmul vf3,
																		  vf1, vf2 vadd vf4, vf4, vf3 addi t1, 16 // next four numbers
																		  ...
																		  3, kLoop nop
																				 // Now vf4 contains 4 sums to be added together.
																				 vaddx vf5,
																		  vf4, vf4x vaddy vf5, vf4, vf5y vaddz vf5, vf4, vf5z // So the sum is in the w.
																															 vmul vf5,
																		  vf5, vf0 // Now it's 3 zeroes and the sum in the w slot.
																				   // Now rotate into place.
																				   and t6,
																		  t5, 0x3 //	where is the column in 4-aligned data?
																				  and t8,
																		  ...
																		  vf5 // X
																			  // b emptycase				// just store, don't add to what's there.
																			  // nop
																			  esccase : lqc2 vf6, 0(t8)vsub vf6, vf6, vf5							  // add to what's there already.
																														  emptycase : sqc2 vf6, 0(t8) // store in t0
																																	  blez t3,
																																	  NoTail
																																		  Tail : lwc1 $f4, 0(t0)			// Put the sum in f4
																																				 TailLoop : lwc1 $f1, 0(t1) // Load 4 numbers into vf1
																																							lwc1 $f2,
																																							0(t2) // Load 4 numbers into vf2
																																							mul.s $f3,
																																							$f1, $f2 sub.s $f4, $f4, $f3 addi t1, 4 // next four numbers
																																							addi t3,
																																							-1 // counter-=4
																																							bgtz t3,
																																							TailLoop
																																								addi t2,
																																							4 swc1 $f4, 0(t0)NoTail : addi t9, 4 // Next Value of V2
																																																   add %
																																																   1,
																																													  % 1, % 5 // Next Row of M1
																																															   addi t4,
...
void platform::math::InsertNegativeVectorAsRow(Matrix &M, const Vector3 &v, const unsigned row)
{
	...
	asm __volatile__("
						 .set noreorder
							 add t0,
					 zero, 3 add t2, zero, % 1 lw t1, 0(% 0) add t3, zero, % 2 // t3=row
																			   lw t4,
					 8(% 0) // t4=4
					 muli t4,
					 t4, 4 mul t4, t4, t3 // row*MStride
										   add t1,
					 t4
						 vsub vf2,
					 vf0, vf0 vsubw vf2, vf2, vf0w Loop : lqc2 vf1, 0(t2)vmul vf1, vf1, vf2 sqc2 vf1, 0(t1)addi t2, 16 addi t0, -4 bgtz t0, Loop addi t1, 16 .set reorder "					  : 
...
void platform::math::AddVectorToRow(Matrix &M, unsigned Row, Vector3 &V)
{
	...
	asm __volatile__("
						 .set noreorder
							 add t0,
					 zero, % 0 // t0= number of 4-wide columns
							   blez t0,
					 Escape // for zero width matrix
						 add t1,
					 zero, % 1 // t1= address of matrix 1's row
							   add t2,
					 zero, % 2 ble t0, 3, Tail iLoop : lqc2 vf1, 0(t1) // Load 4 numbers into vf1
													   lqc2 vf2,
													   0(t2) // Load 4 numbers into vf2
													   vadd vf1,
													   vf1, vf2 sqc2 vf1, 0(t1)addi t0, -1 // counter-=4
																						addi t1,
													   16 // next four numbers
													   bgt t0,
													   3, iLoop addi t2, 16 blez t0, Escape Tail:tLoop : lwc1 f1, 0(t1) // Load a number into f1
																										 lwc1 f2,
																										 0(t2) // Load a number into f2
																										 add.s $f1,
																										 $f1, $f2 swc1 $f1, 0(t1)addi t1, 4 // next four numbers
																										 addi t0,
																										 -1 // counter-=4
																										 bgtz t0,
																										 tLoop
...
void platform::math::AddRow(Vector3 &V1, Matrix &M2, unsigned Row2)
{
	...
	asm __volatile__("
					 ...
					 zero, % 1 // t1= address of matrix 1's row
							   add t2,
					 zero, % 2 blez t0, Escape // for zero width matrix
											ble t0,
					 3, iLoop2 nop iLoop : lqc2 vf1, 0(t1) // Load 4 numbers into vf1
										   lqc2 vf2,
										   0(t2) // Load 4 numbers into vf2
										   vadd vf1,
										   vf1, vf2 sqc2 vf1, 0(t1)addi t1, 16 // next four numbers
										   addi t0,
										   -4 // counter-=4
										   bgt t0,
										   3, iLoop addi t2, 16 blez t0, Escape					  // for zero width matrix
																			 iLoop2 : addi t0, -1 // counter-=4
																							   lwc1 $f1,
																					  0(% 1) // Load 4 numbers into vf1
																					  lwc1 $f2,
																					  0(% 2) // Load 4 numbers into vf2
																					  add.s $f1,
																					  $f1, $f2 swc1 $f1, 0(% 1) addi % 1, 4 // next four numbers
																					  bgtz t0,
																					  iLoop2
																							  addi %
																						  2,
...
void platform::math::RowToVector(const Matrix &M, Vector3 &V, unsigned Row)
{
	...
	asm __volatile__("
					 ...
					 zero, % 2 blez t0, Escape // for zero width matrix
											ble t0,
					 3, Tail nop iLoop : lqc2 vf1, 0(t1) // Load 4 numbers into vf1
										 sqc2 vf1,
										 0(t2) // Load 4 numbers into vector
										 addi t1,
										 16 // next four numbers
										 addi t0,
										 -4 // counter-=4
										 bgt t0,
										 3, iLoop addi t2, 16 blez t0, Escape Tail:tLoop : addi t0, -1 // counter-=4
																									lwc1 f1,
																						   0(t1) // Load a number into f1
																						   swc1 f1,
																						   0(t2)
																							   addi t1,
																						   4 // next four numbers
...
float platform::math::MatrixRowTimesVector(const Matrix &M, const unsigned Row, const Vector3 &V)
{
	...
	asm __volatile__("
					 ...
					 Escape
						 nop
							 sub.s f4,
					 f4, f4 ble % 0, 3, Loop2 nop vsub vf4, vf0, vf0 Loop : addi % 0, -4 // counter-=4
																					  lqc2 vf1,
																			0(% 1) // Load 4 numbers into vf1
																			lqc2 vf2,
																			0(% 2) // Load 4 numbers into vf2
																			vmul vf3,
																			vf1, vf2 vadd vf4, vf4, vf3 addi % 1, 16 // next four numbers
																													  bgt %
																													  0,
																			3, Loop addi % 2, 16
																			// Now vf4 contains 4 sums to be added together.
																			vaddw vf5,
																			vf4, vf4w vaddz vf5, vf4, vf5z vaddy vf5, vf4, vf5y sqc2 vf5, 0(% 3) blez % 0, Escape nop lwc1 f4, 0(% 3) Loop2 : lwc1 f1, 0(% 1) // Load a number into vf1
																																															  lwc1 f2,
																																															  0(% 2) // Load a number into vf2
																																															  mul.s f3,
																																															  f1, f2 add.s f4, f4, f3 addi % 1, 4 // next four numbers
																																																									addi %
																																																									0,
																																															  -1 // counter-=4
																																																  bgtz %
																																															  ...
																																															  Loop2
																																																	  addi %
																																																  2,
																																															  4 swc1 f4, 0(% 3) xor t0, t0 sw t0, 12(% 3) Escape : "					:  
...
void platform::math::TransposeMultiply4(Vector3 &V2, const Matrix4x4 &M, const Vector3 &V1)
{
	...
	asm __volatile__("
					 ...
					 48(% 0)
						 vmulax.xyzw ACC,
					 vf03, vf01 //	apply matrix
							   vmadday.xyzw ACC,
					 vf04, vf01 vmaddaz.xyzw ACC, vf05, vf01 vmaddw.xyzw vf02, vf06, vf00 // using w of vf0 so we multiply by 1!
																						 sqc2 vf02,
					 0(% 2) "					  : /* Outputs. */
...
void platform::math::TransposeMultiply3(Vector3 &V2, const Matrix4x4 &M, const Vector3 &V1)
{
	...
	asm __volatile__("
					 ...
					 vf03, vf01 //	apply matrix
							   vmadday.xyzw ACC,
					 vf04, vf01 vmaddz.xyzw vf02, vf05, vf01 sqc2 vf02, 0(% 2)
