...
set( PLATFORM_CPP_VERSION 23 CACHE STRING "Set the C++ version to compile." )
...
if(PLATFORM_WINDOWS)
	...
	# MSVC does not accept /std:c++23; values above 20 must use /std:c++latest.
	if(PLATFORM_CPP_VERSION GREATER 20)
		set(PLATFORM_MSVC_STD_FLAG "/std:c++latest")
	else()
		set(PLATFORM_MSVC_STD_FLAG "/std:c++${PLATFORM_CPP_VERSION}")
	endif()
	set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} ${PLATFORM_MSVC_STD_FLAG} /Zc:__cplusplus /utf-8")
...
elseif(PLATFORM_SWITCH)
	set(PLATFORM_GRAPHICS_APIS ${PLATFORM_GRAPHICS_APIS} Switch CACHE STRING "" FORCE)
	set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -std:c++${PLATFORM_CPP_VERSION}")
...
elseif(PLATFORM_LINUX)
	...
	set(DYNAMIC_LINK_SUFFIX "_MD")
	set(SIMUL_DYNAMIC_LIBS OFF)
	set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -std=c++${PLATFORM_CPP_VERSION} -DPLATFORM_UNIX=1")
...
elseif(${CMAKE_SYSTEM_NAME} MATCHES "Darwin")
	include_directories( ${SIMUL_PLATFORM_DIR}/MacOS )
	set(CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG} -D_DEBUG -DDEBUG")
	set(STATIC_LINK_SUFFIX "_MT")
	set(DYNAMIC_LINK_SUFFIX "_MD")
	set(SIMUL_DYNAMIC_LIBS OFF)
	set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -std=c++${PLATFORM_CPP_VERSION} -DPLATFORM_UNIX=1")
	set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -L/usr/lib -Wl,-rpath,/usr/lib")
elseif(${CMAKE_SYSTEM_NAME} MATCHES "Emscripten")
	...
	set(CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG} -D_DEBUG -DDEBUG -DPLATFORM_UNIX=1")
	set(CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE} -DPLATFORM_UNIX=1")
	set(STATIC_LINK_SUFFIX "_MT")
	set(DYNAMIC_LINK_SUFFIX "_MD")
	set(SIMUL_DYNAMIC_LIBS OFF )
else()
...
function(LibraryDefaults targname)
	...
	target_compile_definitions( ${targname} PUBLIC _SILENCE_STDEXT_ARR_ITERS_DEPRECATION_WARNING)
	...
	elseif(PLATFORM_EMSCRIPTEN)
		...
		target_compile_options(${targname} PRIVATE -Wall -Wextra -Wno-format-pedantic -Wno-keyword-macro -Wno-ignored-qualifiers -Wno-unused-function -Wno-sign-compare -Wno-gnu-anonymous-struct -Wno-gnu-redeclared-enum -Wno-nested-anon-types -Wno-gnu-zero-variadic-macro-arguments -Wno-unused-parameter -Wno-unused-variable -Wno-unused-value -Wno-missing-braces -Wno-keyword-macro -Wno-language-extension-token -Wno-missing-field-initializers)
	else()
...
function(ImportedLibraryDefaults target)
	...
	target_compile_definitions( ${target} INTERFACE _SILENCE_STDEXT_ARR_ITERS_DEPRECATION_WARNING)
endfunction()
...
function(add_dll libname)
	...
	elseif(PLATFORM_SWITCH)
		set_target_properties( ${target} PROPERTIES SUFFIX ".nro" )
	elseif(PLATFORM_XBOXONE)
		set_target_runtime(${target} dynamic)
