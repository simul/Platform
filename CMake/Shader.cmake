...
function( add_fx_shader_project targetName )
	if(SIMUL_BUILD_SHADERS)
		cmake_parse_arguments(fx "" "OUTPUT;FOLDER" "INCLUDES;SOURCES;OPTIONS;DEFINES" ${ARGN} )
		if("${fx_FOLDER}" STREQUAL "")
			set(fx_FOLDER Shaders)
		endif()
		set(SET_DEFINES /D\"PLATFORM=${SIMUL_PLATFORM_DIR}\" )
		foreach(opt_d ${fx_DEFINES})
			set(SET_DEFINES "${SET_DEFINES} /D${opt_d}" )
		endforeach() 
		set(INCLUDE_OPTS)
		...
		set(outputs${targetName})
		set(EXTRA_OPTS)
		foreach(opt_in ${fx_OPTIONS})
			set(EXTRA_OPTS "${EXTRA_OPTS} ${opt_in}" )
		endforeach()
		set(EXTRA_OPTS "${EXTRA_OPTS} ${SET_DEFINES}")
		separate_arguments(EXTRA_OPTS_S WINDOWS_COMMAND "${EXTRA_OPTS}")
		if(SIMUL_DEBUG_SHADERS)
			set(EXTRA_OPTS_S "${EXTRA_OPTS_S} /Zi /Od")
		endif()
		...
		foreach(in_f ${fx_SOURCES})
			...
			if(NOT ${pos} EQUAL -1)
				...
				add_custom_command(OUTPUT ${out_f}
					COMMAND ${SIMUL_FX_EXECUTABLE} ${EXTRA_OPTS_S} /E"main" ${INCLUDE_OPTS} /Fo"${out_f}" /T"fx_5_0" /nologo ${in_f}
					MAIN_DEPENDENCY ${in_f}
					WORKING_DIRECTORY ${out_folder}
					COMMENT "${SIMUL_FX_EXECUTABLE} ${EXTRA_OPTS_S} /E\"main\" /I\"${fx_INCLUDES}\" /Fo\"${out_f}\" /T fx_5_0 /nologo ${in_f}"
					)
				list(APPEND outputs${targetName} ${out_f})
			else()
				if(MSVC)
