...
void SphericalHarmonics::CopyMip(GraphicsDeviceContext &deviceContext,Texture *tex,int face,int src_mip,float blend)
{
	...
	lightProbesEffect->SetTexture(deviceContext, "sourceCubemap", tex, {TextureAspectFlags::COLOUR, 0, 1, 0, -1});
	// The target is the (i+1)'th mip of the faceIndex face.
	tex->activateRenderTarget(deviceContext, { tex->GetShaderResourceTypeForRTVAndDSV(), { TextureAspectFlags::COLOUR, src_mip + 1, 1, face, 1}});
...
void SphericalHarmonics::RenderEnvmap(GraphicsDeviceContext &deviceContext,crossplatform::Texture *target_texture,int cubemapIndex,float blend)
{
	...
	{
		target_texture->activateRenderTarget(deviceContext, { target_texture->GetShaderResourceTypeForRTVAndDSV(), { TextureAspectFlags::COLOUR, 0, 1, i, 1}});
