...
namespace platform
{
	namespace crossplatform
	{
		...
		*/
		...
		{
		public:
			Shader() : textureSlots(0), textureSlotsForSB(0), rwTextureSlots(0), rwTextureSlotsForSB(0), constantBufferSlots(0), samplerSlots(0), renderPlatform(nullptr)
			{
			}
			virtual ~Shader() {}
		...
		public:
			...
			static void LoadDefaultSamplers(crossplatform::DeviceContext &deviceContext);
...
//! Ensure it's built and up-to-date. Returns false if a required shader compilation fails.
		...
		class ConstantBufferWithSlot : public T
		{
		public:
			static const int bindingIndex = slot;
		};
		template <class T, ResourceUsageFrequency F = ResourceUsageFrequency::MANY_PER_FRAME>
		class ConstantBuffer : public ConstantBufferBase, public T
		{
		public:
