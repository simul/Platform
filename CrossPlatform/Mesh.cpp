...
template <class T>
inline void VectorDelete(std::vector<T> &vec)
{
...
Mesh::Mesh(crossplatform::RenderPlatform *r)
	: done_begin(false), renderPlatform(r), mHasNormal(false), mHasUV(false), mAllByControlPoint(true), stride(0), indexSize(0), numVertices(0), numIndices(0), vertexBuffer(NULL), indexBuffer(NULL), layout(NULL)
{
}
...
bool Mesh::Initialize(crossplatform::RenderPlatform *r, int lPolygonVertexCount, const float *lVertices, const float *lNormals, const float *lUVs, int lPolygonCount, const unsigned int *lIndices, const unsigned short *sIndices)
{
	renderPlatform = r;
...
Mesh::SubMesh *Mesh::SetSubMesh(int submesh, int index_start, int num_indices, std::shared_ptr<Material> m, int lowest, int highest)
{
	if (lowest == -1)
		lowest = 0;
	if (highest == -1)
