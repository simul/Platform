...
static void ConvertMaterial(RenderPlatform *renderPlatform, std::shared_ptr<Material> M, const aiMaterial *m)
{
	...
	{
		unsigned int num = m->GetTextureCount((aiTextureType)t);
		
		#if 1 //PLATFORM_DEBUG_MATERIAL_LOAD
		if (num > 0)
	...
	//Albedo
	if (m->GetTexture(aiTextureType_DIFFUSE, 0, &texturePath, &mapping, &uvindex, &blend) == aiReturn_SUCCESS)
	{
	...
	//Normals
	if (m->GetTexture(aiTextureType_NORMALS, 0, &texturePath, &mapping, &uvindex, &blend) == aiReturn_SUCCESS)
	{
		Texture *T = renderPlatform->GetOrCreateTexture(texturePath.C_Str(), true);
		M->normal.texture = T;
	}
	...
	//Metal
	if (m->GetTexture(aiTextureType_METALNESS, 0, &texturePath, &mapping, &uvindex, &blend) == aiReturn_SUCCESS)
	{
	...
	//Ambient Occlusion
	if (m->GetTexture(aiTextureType_AMBIENT_OCCLUSION, 0, &texturePath, &mapping, &uvindex, &blend) == aiReturn_SUCCESS)
	{
		Texture *T = renderPlatform->GetOrCreateTexture(texturePath.C_Str());
		M->ambientOcclusion.texture = T;
	}
	else if (m->GetTexture(aiTextureType_LIGHTMAP, 0, &texturePath, &mapping, &uvindex, &blend) == aiReturn_SUCCESS)
	{
		Texture *T = renderPlatform->GetOrCreateTexture(texturePath.C_Str());
		M->ambientOcclusion.texture = T;
	}
	...
	//Emissive
	if (m->GetTexture(aiTextureType_EMISSIVE, 0, &texturePath, &mapping, &uvindex, &blend) == aiReturn_SUCCESS)
	{
		Texture *T = renderPlatform->GetOrCreateTexture(texturePath.C_Str());
	...
	M->roughness.value = std::max(1.0f - Float("$mat.shininess", 1.0f), 0.0f);
}
...
void Mesh::Load(const char *filenameUtf8, float scale, AxesStandard fromStandard)
{
	...
	if (!valid)
		valid = core::DynamicLibrary(ASSIMP_LIBNAME);
	if (!valid)
		return;
	...
	for (unsigned i = 0; i < scene->mNumMaterials; i++)
	{
		...
		materials.push_back(renderPlatform->GetOrCreateMaterial(platform::core::QuickFormat("%s", name.C_Str())));
		ConvertMaterial(renderPlatform, materials.back(), m);
	}
	if (pathsplit.size())
