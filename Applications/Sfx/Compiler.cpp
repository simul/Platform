...
wstring BuildCompileCommand(std::shared_ptr<ShaderInstance> shaderInstance, const SfxConfig &sfxConfig, const SfxOptions &sfxOptions, wstring targetDir, wstring outputFile, wstring generatedSourceFilename, ShaderType t, PixelOutputFormat pixelOutputFormat)
{
...
bool RewriteOutput(const SfxConfig &sfxConfig, const SfxOptions &sfxOptions, string sourcePathUtf8, const map<int, string> &fileList, ostringstream *log, string str)
{
...
int Compile(std::shared_ptr<ShaderInstance> shaderInstance, const string &sourceFile, string targetFile, ShaderType t, PixelOutputFormat pixelOutputFormat, const string &sharedSource, ostringstream &sLog, const SfxConfig &sfxConfig, const SfxOptions &sfxOptions, map<int, string> fileList, std::ofstream &combinedBinary, BinaryMap &binaryMap, const Declaration *rtState)
{
	...
	case COMPUTE_SHADER:
		...
		break;
	case MESH_SHADER:
		shaderTypeSuffix = L"m";
		break;
	case AMPLIFICATION_SHADER:
		shaderTypeSuffix = L"a";
		break;
	...
	{
		if (sfxConfig.meshPreamble.empty())
		{
			preamble += "#define USE_MESH_SHADER 1\n";
		}
	}
	else if (t == AMPLIFICATION_SHADER)
	{
		if (sfxConfig.amplificationPreamble.empty())
		{
			preamble += "#define USE_AMPLIFICATION_SHADER 1\n";
		}
	}
