...
void Framebuffer::InitVulkanFramebuffer(crossplatform::GraphicsDeviceContext &deviceContext)
{
	...
	if(buffer_depth_texture)
		vulkanRenderPlatform->CreateVulkanRenderpass(deviceContext,mDummyRenderPasses[RPType::DEPTH],0,nullptr,depth_format,numAntialiasingSamples);
	...
	if(buffer_depth_texture)
		attachments[1] = *(buffer_depth_texture->AsVulkanImageView({ crossplatform::ShaderResourceType::TEXTURE_2D, {0, 1, 0, 1} }));
	...
	for(int i=1;i<8;i++)
	{
		...
		for(int j=0;j<totalNum;j++)
		{
			...
			attachments[0]=*(buffer_texture->AsVulkanImageView({ crossplatform::ShaderResourceType::TEXTURE_2D, {0, 1, j, 1} }));
			vk::Result res = vulkanDevice->createFramebuffer(&framebufferCreateInfo, nullptr, &mFramebuffers[i][j]);
			SIMUL_ASSERT(res==vk::Result::eSuccess);
