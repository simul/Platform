...
void Framebuffer::InitVulkanFramebuffer(crossplatform::GraphicsDeviceContext &deviceContext)
{
	...
	for (int i = 1; i < 8; i++)
	{
		...
		for (int j = 0; j < totalNum; j++)
		{
			...
			attachments[0] = *(buffer_texture->AsVulkanImageView(MakeTextureView(crossplatform::ShaderResourceType::TEXTURE_2D,crossplatform::TextureAspectFlags::COLOUR, 0, 1, j, 1)));
