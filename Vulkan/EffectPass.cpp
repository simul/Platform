...
void EffectPass::ApplyContextState(crossplatform::DeviceContext &deviceContext, vk::DescriptorSet &descriptorSet)
{
	...
	const crossplatform::RenderState *currentBlendState;
	const crossplatform::RenderState *currentDepthStencilState;
	const crossplatform::RenderState *currentRasterizerState;
	if (blendState)
	{
		deviceContext.renderPlatform->SetRenderState(deviceContext, blendState);
	}
	currentBlendState = deviceContext.contextState.blendState;
	if (depthStencilState)
	{
		deviceContext.renderPlatform->SetRenderState(deviceContext, depthStencilState);
	}
	currentDepthStencilState = deviceContext.contextState.depthState;
	if (rasterizerState)
	{
		deviceContext.renderPlatform->SetRenderState(deviceContext, rasterizerState);
	}
	currentRasterizerState = deviceContext.contextState.rasterizerState;
	...
	else
	{
		...
		hashval = MakeRenderPassHash(pixelFormat, numOfSamples, appliedTopology, cs->currentLayout, currentBlendState, currentDepthStencilState, currentRasterizerState, multiview);
		const auto &p = m_RenderPasses.find(hashval);
		if (p == m_RenderPasses.end())
		{
			renderPassPipeline = &m_RenderPasses[hashval];
			InitializePipeline(*graphicsDeviceContext, renderPassPipeline, pixelFormat, numOfSamples, appliedTopology, currentBlendState, currentDepthStencilState, currentRasterizerState, multiview);
		}
		else
			renderPassPipeline = &(m_RenderPasses[hashval]);
		...
		if (tv->depthTarget.texture && depthStencilState)
		{
			...
			if (dt.texture && (depthWrite || depthTest))
			{
				...
				vk::ImageLayout layout = depthWrite ? vk::ImageLayout::eDepthStencilAttachmentOptimal : depthTest ? vk::ImageLayout::eDepthStencilReadOnlyOptimal
...
void EffectPass::CreateDescriptorPoolAndSetLayoutAndPipelineLayout()
{
	...
	int numDescriptors = numResourceSlots + numRwResourceSlots + numSbResourceSlots + numRwSbResourceSlots + numSamplerResourceSlots + numConstantBufferResourceSlots + numResourceSlots;
...
void EffectPass::InitializePipeline(crossplatform::DeviceContext &deviceContext, RenderPassPipeline *renderPassPipeline, crossplatform::PixelFormat pixelFormat, int numOfSamples, crossplatform::Topology topology, const crossplatform::RenderState *blendState, const crossplatform::RenderState *depthStencilState, const crossplatform::RenderState *rasterizerState, bool multiview)
{
	vulkan::RenderPlatform* vulkanRenderPlatform = (vulkan::RenderPlatform*)renderPlatform;
	vk::Device *vulkanDevice = vulkanRenderPlatform->AsVulkanDevice();
	...
	{
		// Mesh shader pipeline: task + mesh + pixel
		// Task shader (amplification) is optional
		std::vector<vk::PipelineShaderStageCreateInfo> shaderStages;

		// Add task shader (amplification) if present
		if (as)
		{
			shaderStages.push_back(vk::PipelineShaderStageCreateInfo()
									   .setStage(vk::ShaderStageFlagBits::eTaskEXT)
									   .setModule(as->mShader)
									   .setPName(as->entryPoint.c_str()));
		}

		// Add mesh shader
		shaderStages.push_back(vk::PipelineShaderStageCreateInfo()
								   .setStage(vk::ShaderStageFlagBits::eMeshEXT)
								   .setModule(ms->mShader)
								   .setPName(ms->entryPoint.c_str()));

		// Add pixel/fragment shader
		shaderStages.push_back(vk::PipelineShaderStageCreateInfo()
								   .setStage(vk::ShaderStageFlagBits::eFragment)
								   .setModule(f->mShader)
								   .setPName(f->entryPoint.c_str()));
...
EffectPass::RenderPassPipeline &EffectPass::GetRenderPassPipeline(crossplatform::DeviceContext &deviceContext)
{
	crossplatform::ContextState *cs = &deviceContext.contextState;
	...
	{
		pixelFormat = vulkan::RenderPlatform::GetActivePixelFormat(*deviceContext.AsGraphicsDeviceContext());
		numOfSamples = vulkan::RenderPlatform::GetActiveNumOfSamples(*deviceContext.AsGraphicsDeviceContext());
	}
	crossplatform::Topology appliedTopology = (cs->topology != crossplatform::Topology::UNDEFINED) ? cs->topology : topology;
...
RenderPassHash EffectPass::MakeRenderPassHash(crossplatform::PixelFormat pixelFormat, int numOfSamples, crossplatform::Topology topology, const crossplatform::Layout *layout, const crossplatform::RenderState *blendState, const crossplatform::RenderState *depthStencilState, const crossplatform::RenderState *rasterizerState, bool multiview)
{
