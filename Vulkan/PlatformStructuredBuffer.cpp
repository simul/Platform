...
PlatformStructuredBuffer::PlatformStructuredBuffer()
	: mSlots(0), mMaxApplyCount(2), mCurApplyCount(0), last_offset(0), buffer(nullptr), mCpuRead(false), mSetCPUData(false), mLastFrame(0), mFrameIndex(0)
{
}
...
{
	bufferUsageHint = _bufferUsageHint;
	renderPlatform = r;
	mNumElements = count;
	...
	if (mCpuRead)
	{
		...
		{
			vulkanRenderPlatform->CreateVulkanBuffer(nullptr, buffer_aligned_size, usageFlags, vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent, mReadBuffers[i], mReadBufferAllocationInfo[i], (name + "psb read").c_str());
		}
	}
