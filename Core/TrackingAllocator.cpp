...
static string memstring(size_t bytes)
{
	string memtxt;
	if(bytes<1024)
		memtxt+=std::format("{0} bytes",bytes);
	else
	{
		size_t k=bytes/1024;
		if(k<1024)
		{
			memtxt+=std::format("{0} kb",k);
		}
		else
		{
			size_t meg=k/1024;
			memtxt+=std::format("{0} Mb",meg);
		}
	}
	return memtxt;
}
